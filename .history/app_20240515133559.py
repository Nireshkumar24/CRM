...
from flask import Flask, jsonify, render_template, request, redirect, send_from_directory, url_for,flash,session,send_file
...
#     return render_template('adduser.html',customers=all_customers)
@app.route('/adduser', methods=['GET', 'POST'])
def adduser():
    if request.method == 'POST':
        try:
...
def edit_customer(id):
    ...
    customer.mobileNumber=request.form['mobileNumber']
...
def get_followup_notification_count():
    ...
    # Calculate the end of the notification period (e.g., 3 days from now)
    notification_end_date = today + timedelta(days=3)
    # Query the database for activities with follow-up dates within the specified timeframe
